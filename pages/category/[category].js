...
const category = ({movies,categories,types,categorie}) => {
 ...
 <Head>
  <title>{categorie.data[0].attributes.Title}</title>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0"></meta>
      ...
      >
        <Toolbar />
        <h2 className='flex font-bold text-3xl mb-10 text-white align-center'> {categorie.data[0].attributes.Title}</h2>
    ...
    const options = {
        ...
        categories: {
             // Slug ในหน้าCategory ตามค่า Strapi ที่เราตั้งชื่อไว้ ดูด้วยว่าเป็นตัวพิมเล็กหรือพิมใหญ่
                Slug: query.category,//queryหน้า[category] กับใส่ ตรง reretion ใหเหมือนกัน ในกรณีต้องการมีหลายหมวดหมู่
            },
        },
        pagination: {
...
//**********************End types*******************************************
 ...
 const queryCategory = qs.stringify({
    ...
    //ตรง Slug Strapi
      Slug: {
        $eq: query.category, //queryหน้า[category]
    }
      },
  });
  
  const resTitle = await fetch(`${process.env.API_BASE_URL}/api/categories?${queryCategory}`,{
  headers: { //เข้า API ผ่านรหัส
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${process.env.BACKEND_API_KEY}`  
  },
  });
  const categorie = await resTitle.json() //part
  //**********************end TitleCategory***********************************************
